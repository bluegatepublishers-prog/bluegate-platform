...
export default async function BookDetailsPage({ params }: PageProps) {
  const { slug } = await params;
  const dbBook = await prisma.book.findFirst({ where: { slug, published: true }, include: { class: true, subject: true, series: true } });
  if (!dbBook) notFound();
  const mapBook = (item: typeof dbBook) => ({ id: item.id, slug: item.slug, title: item.title, author: item.author ?? "", subtitle: item.subtitle ?? "", class: item.class.name, board: item.board ?? "", subject: item.subject.name, series: item.series?.name ?? "", isbn: item.isbn ?? "", pages: item.pages ?? 0, cover: item.coverImage || "/images/book-placeholder.jpg", pdf: item.samplePdf || "", description: item.description ?? item.aboutBook ?? "", featured: item.featured, edition: item.edition ?? "", publisher: item.publisher ?? "", price: item.price?.toString() ?? "" });
  const book = mapBook(dbBook);
  const related = await prisma.book.findMany({ where: { published: true, subjectId: dbBook.subjectId, NOT: { id: dbBook.id } }, include: { class: true, subject: true, series: true }, take: 4 });
