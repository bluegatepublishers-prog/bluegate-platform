...
type GenerateQuestionPaperInput = { generationId: string; bookId: string; chapterIds: string[]; title: string; totalMarks: number; duration: number; difficulty: "Easy" | "Medium" | "Hard"; autoDistribution: boolean; questionTypes: string[]; questionCounts: Record<string, number>; estimatedQuestions: number };
...
export async function generateQuestionPaper(input: GenerateQuestionPaperInput) {
  const teacher = await requireTeacher();
  if (!input || typeof input.generationId !== "string" || !/^[a-zA-Z0-9_-]{16,64}$/.test(input.generationId) || typeof input.bookId !== "string" || !input.bookId || !Array.isArray(input.chapterIds) || !input.chapterIds.length || input.chapterIds.some(id => typeof id !== "string" || !id) || new Set(input.chapterIds).size !== input.chapterIds.length || typeof input.title !== "string" || !input.title.trim() || input.title.length > 160 || !Number.isInteger(input.totalMarks) || input.totalMarks < 1 || input.totalMarks > 500 || !Number.isInteger(input.duration) || input.duration < 10 || input.duration > 360 || !["Easy", "Medium", "Hard"].includes(input.difficulty) || typeof input.autoDistribution !== "boolean" || !Array.isArray(input.questionTypes) || !input.questionTypes.length || new Set(input.questionTypes).size !== input.questionTypes.length || input.questionTypes.some(type => !allowedQuestionTypes.has(type))) return { ok: false as const, message: "Please complete all question paper settings." };
  const uniqueChapterIds = [...new Set(input.chapterIds)];
  try {
    const book = await prisma.book.findFirst({ where: { id: input.bookId, published: true }, select: { id: true, title: true, classId: true, subjectId: true, chapters: { where: { id: { in: uniqueChapterIds } }, select: { id: true, title: true, chapterNumber: true, approved: true, reviewedText: true, _count: { select: { learningOutcomes: true } } } } } });
    ...
    if (book.chapters.some(chapter => !chapter.approved || !chapter.reviewedText?.trim() || chapter._count.learningOutcomes < 1)) return { ok: false as const, message: "One or more chapters are not ready for AI generation." };
