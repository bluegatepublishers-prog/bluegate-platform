...
export async function GET(_request: Request, { params }: { params: Promise<{ resourceId: string }> }) {
  ...
  const resource = await prisma.publisherTeacherResource.findFirst({ where: { id: resourceId, publisherId: actor.publisherId, archivedAt: null, book: { publisherId: actor.publisherId } }, select: { objectKey: true, originalFileName: true } });
  ...
  const signed = await provider.createSignedDownloadUrl({ key, expiresInSeconds: 60, downloadFilename: resource.originalFileName, disposition: "attachment" });
  ...
  return new NextResponse(source.body, { headers: { ...headers, "Content-Type": "application/pdf", "Content-Disposition": createContentDisposition(resource.originalFileName, "attachment"), ...(source.headers.get("content-length") ? { "Content-Length": source.headers.get("content-length")! } : {}) } });
}
