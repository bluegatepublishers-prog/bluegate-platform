...
export async function POST(request: Request) {
  ...
  const created = await prisma.$transaction(
    async (tx) => {
      ...
      const order = await tx.bookModule.aggregate({
        where: { bookId: book.id, chapterId: chapter.id },
        _max: { displayOrder: true },
      });
      const node = await tx.bookModule.create({
        data: {
          bookId: book.id,
...
async function readPayload(request: Request) {
  ...
  if (
    !bookId ||
    input.parentType !== "CHAPTER" ||
    !parentId
  ) {
    return bad("A Module must belong to a valid Chapter.");
  }
