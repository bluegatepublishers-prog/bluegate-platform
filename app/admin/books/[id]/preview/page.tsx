...
export default async function AdminBookPreview({ params }: { params: Promise<{ id: string }> }) {
  ...
  const book = await prisma.book.findUnique({ where: { id }, include: { class: true, subject: true, series: true } });
  if (!book) notFound();
