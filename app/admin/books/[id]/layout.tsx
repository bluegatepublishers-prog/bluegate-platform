...
export default async function BookAdminLayout({ children, params }: { children: React.ReactNode; params: Promise<{ id: string }> }) {
  ...
  if (!book) notFound();
  const tabs = [["Academic Content", `/admin/books/${id}/content`], ["Details", `/admin/books/${id}/edit`], ["Chapters", `/admin/books/${id}/chapters`], ["Question Bank", `/admin/books/${id}/questions`], ["Learning Outcomes", `/admin/books/${id}/learning-outcomes`], ["Classroom Activities", `/admin/books/${id}/activities`], ["AI Knowledge", `/admin/books/${id}/knowledge`]];
