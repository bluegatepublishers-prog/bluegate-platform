...
export default async function StudentDashboardPage() {
  ...
  const completedRevisions = await getStudentCompletedRevisions();
  ...
  return (
    <main className="space-y-7 p-4 sm:p-6 lg:p-8">
      ...
      <section className="grid gap-5 md:grid-cols-2">
        ...
        <Placeholder icon={CalendarDays} title="School Context" description={`${identity.enrollment.schoolClass.name}, Section ${identity.enrollment.section.name} · ${identity.academicYear.name}`} />
      </section>
    </main>
  );
}
