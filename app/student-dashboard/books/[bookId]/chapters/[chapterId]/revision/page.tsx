...
export default async function StudentRevisionPage({
  ...
  return (
    <main className="space-y-7 p-4 sm:p-6 lg:p-8">
      ...
      <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8"><h2 className="text-2xl font-bold">Ask My Book</h2><p className="mt-2 text-slate-600">Use guided AI tools grounded only in this approved chapter.</p>{assistant.state === "LOCKED" ? <p className="mt-5 rounded-2xl bg-amber-50 p-5 font-semibold text-amber-900">Ask My Book is available with Premium.</p> : assistant.state === "FEATURE_DISABLED" ? <p className="mt-5 rounded-2xl bg-slate-50 p-5 font-semibold text-slate-700">This feature is not available on your platform.</p> : assistant.state === "UNAVAILABLE" ? <p className="mt-5 text-slate-500">Ask My Book is not available for this chapter.</p> : <Link href={`/student-dashboard/books/${book.id}/chapters/${chapter.id}/assistant`} className="mt-5 inline-flex min-h-12 items-center rounded-xl bg-indigo-700 px-6 py-3 font-bold text-white">Open Ask My Book</Link>}</section>
    </main>
  );
}
