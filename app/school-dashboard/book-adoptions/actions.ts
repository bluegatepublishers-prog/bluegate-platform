...
import { BookAdoptionStatus, PlatformFeatureKey } from "@prisma/client";
...
export async function requestBookAdoption(form: FormData) {
  ...
  await requirePublisherFeature(school.publisherId!, PlatformFeatureKey.BOOK_APPROVALS);
...
export async function cancelBookAdoption(id: string) { const school = await requireSchool(); await requirePublisherFeature(school.publisherId!, PlatformFeatureKey.BOOK_APPROVALS); await prisma.schoolBookAdoption.updateMany({ where: { id, schoolId: school.id, status: BookAdoptionStatus.PENDING }, data: { status: BookAdoptionStatus.REVOKED, active: false, revokedAt: new Date(), revokedReason: "Cancelled by school" } }); revalidatePath("/school-dashboard/book-adoptions"); revalidatePath("/school-dashboard"); }
