...
import { Prisma, SchoolOnboardingStatus, TeacherOnboardingStatus } from "@prisma/client";
import { prisma } from "@/lib/prisma";
import { hashPassword } from "@/lib/password";
import { BLUEGATE_PUBLISHER_ID } from "@/lib/publisher-context";
import { cleanText, normalizeActivationCode, normalizeAdmissionNumber, normalizeEmail, sameCalendarDate, validEmail, validatePassword } from "./onboarding-policy";
...
export async function createPendingSchool(input: Record<string, unknown>) {
  ...
  if (!schoolName || !principalName || !validEmail(email) || !phone || !address || !city || !state || !/^\d{6}$/.test(pincode) || passwordError) throw new OnboardingError(passwordError ?? "Complete every required field with valid details.");
  const publisher = await prisma.publisher.findFirst({ where: { id: BLUEGATE_PUBLISHER_ID, active: true }, select: { id: true } });
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      return tx.school.create({ data: { userId: user.id, publisherId: publisher.id, schoolName, principalName, address, city, state, pincode, status: SchoolOnboardingStatus.PENDING }, select: { id: true } });
    });
...
export async function createPendingTeacher(input: Record<string, unknown>) {
  ...
  const passwordError = validatePassword(input.password, input.confirmPassword);
  if (!name || !validEmail(email) || !phone || !schoolId || !designation || !subject || !classes || passwordError) throw new OnboardingError(passwordError ?? "Complete every required field with valid details.");
  const school = await prisma.school.findFirst({ where: { id: schoolId, status: SchoolOnboardingStatus.APPROVED, publisher: { active: true } }, select: { id: true, schoolName: true, publisherId: true } });
  if (!school?.publisherId || await prisma.user.findUnique({ where: { email }, select: { id: true } })) throw new OnboardingError(unavailable);
  const password = await hashPassword(String(input.password));
  try {
    return await prisma.$transaction(async (tx) => {
      const user = await tx.user.create({ data: { name, email, phone, password, role: "TEACHER", publisherId: school.publisherId } });
      ...
      return tx.teacherSchoolRequest.create({ data: { teacherId: teacher.id, schoolId: school.id, publisherId: school.publisherId!, activeKey: `${teacher.id}:${school.id}` }, select: { id: true } });
    });
  } catch (error) { if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") throw new OnboardingError(unavailable); throw error; }
}

export async function activateStudentAccount(input: Record<string, unknown>) {
  const code = normalizeActivationCode(input.activationCode), admissionNumber = normalizeAdmissionNumber(input.admissionNumber), dateOfBirth = cleanText(input.dateOfBirth, 10);
  const passwordError = validatePassword(input.password, input.confirmPassword);
  if (code.length !== 12 || !admissionNumber || passwordError) throw new OnboardingError(passwordError ?? unavailable);
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const user = await tx.user.create({ data: { name: activation.student.name, email, password, role: "STUDENT", publisherId: activation.school.publisherId } });
    ...
    await tx.studentActivationCode.update({ where: { id: activation.id }, data: { usedAt: now, usedByUserId: user.id } });
    return { email };
