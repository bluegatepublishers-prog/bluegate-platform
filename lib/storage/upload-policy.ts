...
export function publisherUploadPath(publisherId:string,scope:UploadScope,name:string,schoolId?:string){const safePublisher=publisherId.replace(/[^a-zA-Z0-9_-]/g,"");if(!safePublisher)throw new Error("A valid publisher context is required.");if(scope==="school-logo"){const safeSchool=schoolId?.replace(/[^a-zA-Z0-9_-]/g,"");if(!safeSchool)throw new Error("A valid school context is required.");return `publishers/${safePublisher}/schools/${safeSchool}/logo/${sanitizeUploadFilename(name)}`;}return `publishers/${safePublisher}/${uploadPrefixForScope(scope)}/${sanitizeUploadFilename(name)}`;}
export function isPublisherUploadUrl(value:string|null|undefined,publisherId:string,scopes:UploadScope[]){if(!value)return true;try{const parsed=new URL(value),safePublisher=publisherId.replace(/[^a-zA-Z0-9_-]/g,"");if(parsed.protocol!=="https:"||!parsed.hostname.endsWith(".public.blob.vercel-storage.com")||!safePublisher)return false;return scopes.some(scope=>parsed.pathname.startsWith(`/publishers/${safePublisher}/${uploadRules[scope].prefix}/`))}catch{return false}}
