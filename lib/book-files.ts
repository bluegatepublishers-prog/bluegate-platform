...
export function isManagedBookFile(value:string|null|undefined):value is string{if(!isManagedFileUrl(value))return false;if(value.startsWith("/uploads/books/"))return true;try{return /^\/(?:publishers\/[^/]+\/)?books\//.test(new URL(value).pathname)}catch{return false}}
