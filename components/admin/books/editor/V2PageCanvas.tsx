        ...
        >
          <div
            onPointerDown={(event: PointerEvent<HTMLDivElement>) => {
              if (event.target === event.currentTarget) {
                ...
                onClearSelection();
                onSetInsertionPoint?.({
                  x: Math.max(0, Math.min(page.width, (event.clientX - bounds.left) / Math.max(0.01, scale))),
                  y: Math.max(0, Math.min(page.height, (event.clientY - bounds.top) / Math.max(0.01, scale))),
                });
              }
            }}
          ...
          >
            ...
            {showGuides ? (
              <div
                data-v2-page-guides
                className="pointer-events-none absolute z-[999998] border border-dashed border-cyan-500/70"
                style={{ inset: Math.min(48, Math.max(24, Math.min(page.width, page.height) * 0.06)) }}
              />
            ) : null}
            ...
            {visualFrames.map((frame) => isV2MainFlowFrame(frame) ? (
              <div
                ...
                style={{
                  ...
                  width: frame.width,
                  height: frame.height,
                  zIndex: V2_LAYER_ORDER[frame.layer] * 100000 + frame.zIndex,
                  writingMode: "horizontal-tb",
                }}
                onPointerDown={() => onActivateMainFlow?.(frame.id)}
                onFocusCapture={() => onActivateMainFlow?.(frame.id)}
              >
                {renderFrame(frame, visualFrames)}
              </div>
            ) : (
              <V2Frame
                key={frame.id}
                ...
                renderChildren={(visualFrame) => visualFrame.type === "EDUCATIONAL"
                  ? <V2EducationalFrame
                      ...
                      onPayloadChange={(frameId, payload) => onPatchFrame?.(frameId, { payload }, "Educational Block body updated")}
                    />
                  : visualFrame.type === "TEXT" && visualFrame.children?.length
                    ? <V2TextContainerFrame
                        frame={visualFrame}
                        scale={scale}
                        selectedFrameId={selectedFrameId}
                        renderFrame={(child, children) => renderFrame(child, children)}
