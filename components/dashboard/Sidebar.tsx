...
export const teacherNavigation = [
  ...
  {
    name: "Planner",
    href: "/teacher-dashboard/planner",
    icon: CalendarDays,
  },
  {
    name: "Messages",
...
export default function Sidebar({
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-slate-100 bg-white p-5 lg:flex">
      <Link
        href="/teacher-dashboard"
      ...
      >
        ...
        <span className="min-w-0">
          ...
          <span className="block truncate text-[11px] font-bold uppercase tracking-wider text-slate-500">
            {branding.portalTitle || "Teacher Portal"}
          </span>
        </span>
      </Link>
        ...
        {navigationItems.map(({ name, href, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex min-h-14 items-center gap-4 rounded-2xl px-4 font-semibold transition ${
                active
      ...
      <div className="mt-auto rounded-3xl bg-gradient-to-br from-blue-50 to-violet-50 p-5">
        <p className="font-semibold leading-6 text-slate-700">
          Great teachers inspire, empower and build futures.
        </p>
      ...
      <Link
        ...
        className="mt-4 flex items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3 font-semibold text-blue-700"
      >
