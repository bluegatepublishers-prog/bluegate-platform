...
export default function Sidebar({ teacherName, schoolName, branding, features }: SidebarProps) {
  ...
  const navigationItems = teacherNavigation.filter((item) => item.name !== "Planner" || features.PLANNER !== false).filter((item) => item.name !== "Resources" || features.TEACHER_RESOURCES !== false);
