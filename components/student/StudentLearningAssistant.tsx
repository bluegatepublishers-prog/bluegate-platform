...
export default function StudentLearningAssistant({
  ...
  const [history, setHistory] =
    useState(initialData.history);

  const [remaining, setRemaining] =
    useState(
      initialData.quota.remaining,
    );
  ...
  const selectedMode =
    initialData.modes.find(
      (mode) =>
        mode.intent === selectedIntent,
    );
      ...
      if (
        ...
        throw new Error(
          typeof result.message ===
            "string"
            ? result.message
            : "Ask My Book is temporarily unavailable. Please try again.",
        );
      }

      setHistory((current) => [
        ...current,
        result.message as HistoryMessage,
      ]);
    ...
    <>
      <header className="rounded-3xl bg-gradient-to-br from-blue-800 to-indigo-800 p-7 text-white shadow-xl sm:p-10">
        ...
        <p className="mt-6 max-w-3xl leading-7 text-blue-100">
          Ask about this chapter or choose a
          learning tool. Answers come only
          from your approved Smart Book
          chapter.
        </p>
      </header>
          ...
          {initialData.modes.map(
            (mode) => (
              ...
              >
                ...
                <span className="mt-5 block text-lg font-bold">
                  {mode.label}
                </span>
      ...
      <section
        aria-labelledby="chapter-history-heading"
        className="space-y-4"
      >
        <div>
          <h2
            id="chapter-history-heading"
            className="text-2xl font-bold"
          >
          ...
          <p className="mt-2 text-slate-600">
            History is private to your
            account and does not continue
        ...
        {history.length ? (
          history.map((message) => (
            ...
            >
              ...
              <div
                className={`mt-4 whitespace-pre-wrap rounded-2xl p-5 leading-7 ${
                  message.refused
          ...
          <p className="rounded-3xl border border-dashed bg-slate-50 p-8 text-center text-slate-500">
            Choose a learning tool above to
            begin this chapter&apos;s Ask My
            Book conversation.
          </p>
        )}
      </section>
