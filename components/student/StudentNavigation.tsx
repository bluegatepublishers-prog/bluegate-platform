...
import { BookOpen, CalendarDays, Home, Megaphone, School } from "lucide-react";
...
export default function StudentNavigation({ mobile = false }: { mobile?: boolean }) {
  ...
  if (mobile) return <nav aria-label="Student navigation" className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t bg-white/95 p-2 backdrop-blur lg:hidden">{links}</nav>;
