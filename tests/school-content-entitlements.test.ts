...
test("school book and resource entitlements are lifecycle records with tenant-safe uniqueness", () => {
  assert.match(schema, /model SchoolBookEntitlement \{[\s\S]*@@unique\(\[schoolId, bookId\]\)/);
  assert.match(schema, /model SchoolResourceEntitlement \{[\s\S]*@@unique\(\[schoolId, resourceId\]\)/);
  assert.match(schema, /enum ContentEntitlementStatus \{\s+ACTIVE\s+PAUSED\s+REVOKED\s+ARCHIVED/s);
  assert.doesNotMatch(entitlementService, /schoolBookEntitlement\.delete|schoolResourceEntitlement\.delete/);
});
...
test("duplicate active assignment is rejected and publisher ownership is server-derived", () => {
  ...
  assert.match(entitlementService, /existing\.some\(\(item\) => item\.status === ContentEntitlementStatus\.ACTIVE\)/);
  assert.match(entitlementService, /publisherId: actor\.publisherId/);
  assert.match(entitlementService, /isolationLevel: Prisma\.TransactionIsolationLevel\.Serializable/);
});

test("book entitlement lifecycle requires a revoke reason and only ACTIVE is operational", () => {
  assert.deepEqual(
    evaluateContentEntitlementTransition({ current: "ACTIVE", action: "pause" }),
    { allowed: true, next: "PAUSED" },
  );
  assert.deepEqual(
    evaluateContentEntitlementTransition({ current: "PAUSED", action: "resume" }),
    { allowed: true, next: "ACTIVE" },
  );
  assert.deepEqual(
    evaluateContentEntitlementTransition({ current: "ACTIVE", action: "revoke" }),
...
test("resource access requires active school entitlement and active parent book entitlement", () => {
  assert.match(resourcePolicy, /schoolEntitlements: \{ some: \{ schoolId, publisherId, status: "ACTIVE" \} \}/);
  assert.match(resourcePolicy, /book:[\s\S]*schoolEntitlements:[\s\S]*status: "ACTIVE"/);
...
test("entitlement mutations are atomic and audited without recording reasons", () => {
  ...
  assert.match(auditPolicy, /publisher\.school_book_entitlement\.assign/);
  assert.match(auditPolicy, /publisher\.school_resource_entitlement\.revoke/);
  assert.doesNotMatch(entitlementService, /metadata:\s*\{[^}]*reason/s);
});

test("migration is additive, backfills current access, and has no data-loss statements", () => {
  assert.match(migration, /INSERT INTO "SchoolBookEntitlement"/);
  assert.match(migration, /INSERT INTO "SchoolResourceEntitlement"/);
  assert.match(migration, /BookChapter"[\s\S]*"published" = "approved"/);
...
test("existing chapters remain supported while optional top-level parts extend hierarchy", () => {
  ...
  assert.match(schema, /model BookChapter \{[\s\S]*unitId\s+String\?/);
  assert.match(schema, /model BookTopic \{[\s\S]*moduleId\s+String\?/);
...
test("structure ordering, moves, duplication, and archive lifecycle persist behind the single Content Studio", () => {
  ...
  assert.match(structureService, /setBookStructureNodeArchived/);
  assert.doesNotMatch(structureService, /book(Part|Unit|Chapter|Module|Topic)\.delete/);
});
...
test("book features are reusable, unique, ordered, highlighted, and archived instead of deleted", () => {
  ...
  assert.doesNotMatch(featureService, /bookFeatureDefinition\.delete|bookFeatureAssignment\.delete/);
});
...
test("digital content links reuse resource files and support all stable target levels", () => {
  ...
  assert.match(schema, /enum BookContentTargetType \{\s+BOOK\s+PART\s+UNIT\s+CHAPTER\s+MODULE\s+TOPIC/s);
  assert.match(resourceLinks, /resourceId_targetKey/);
  assert.doesNotMatch(resourceLinks, /fileUrl|copyFile|upload/);
...
test("school entitlement pages are responsive management lists without wide tables", () => {
  for (const source of [bookPage, resourcePage]) {
    ...
    assert.doesNotMatch(source, /<table|min-w-\[\d+px\]|overflow-x-auto/);
  }
});

test("school admins cannot mutate publisher content through entitlement routes", () => {
  assert.match(entitlementService, /requireLivePublisherAdmin/);
  assert.doesNotMatch(entitlementService, /requireSchool\(\)/);
  assert.match(structureService, /requireLivePublisherAdmin/);
  assert.match(featureService, /requireLivePublisherAdmin/);
});
